...
export class MirrorRAG {
  ...
  async searchResearch(query: string, limit: number = 3) {
    ...
    console.log(
      `[MirrorRAG] Querying research with dimensions: ${vector.length}`,
    );
    ...
    console.log(
      `[MirrorRAG] Research hits: ${(chunks as ResearchChunkResult[])?.length || 0}`,
    );
  ...
  async searchParallel(userId: string, query: string) {
    ...
    const researchContext =
      research.length > 0
        ? `<research>\n${research.map((c: ResearchChunkResult) => `[${c.author}, ${c.year}]: ${c.content}`).join("\n---\n")}\n</research>`
        : "";
